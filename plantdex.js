...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function displaySearchResults(data, index) {
    ...
    const scientificName =
      plant.scientific_name?.join(", ") || "Scientific Name not available";
    const sunlight =
      plant.sunlight?.[0] || "Sunlight information not available";
    ...
    const defaultMessage = resultElement.querySelector(
  ...
  function displayPlantDetails(plantDetails, plantDetailsElement) {
    ...
    const detailsHTML = `
      ...
      <p><strong>Common Name:</strong> ${
        plantDetails.common_name || "Not available"
      }</p>
      <p><strong>Scientific Name:</strong> ${
        plantDetails.scientific_name?.join(", ") || "Not available"
      }</p>
      <p><strong>Family:</strong> ${plantDetails.family || "Not available"}</p>
      <p><strong>Care Level:</strong> ${
        plantDetails.care_level || "Not available"
      }</p>
      <p><strong>Cycle:</strong> ${plantDetails.cycle || "Not available"}</p>
      <p><strong>Description:</strong> ${
        plantDetails.description || "Not available"
      }</p>
    `;
  ...
  function clearPreviousResults() {
    const searchResultsDisplayLocation =
